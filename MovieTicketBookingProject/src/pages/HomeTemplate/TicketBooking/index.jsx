...
const TicketBooking = () => {
    ...
    return (
      <div className="space-y-2">
        ...
        <p className="text-base font-medium">
          Format:{" "}
          <span className="text-amber-400 font-semibold">2D / IMAX</span>
        </p>
        ...
        <p className="text-base">
          Auditorium:{" "}
          <span className="text-amber-400 font-semibold">{tenRap}</span>
        </p>
      </div>
    );
  };
  ...
  const renderInfoSeatSelected = () => {
    if (activeSeats.length > 0) {
      return activeSeats.map(seat => seat.tenGhe).join(", ")
    } else {
  ...
  const calculateSubTotal = () => {
    return activeSeats.reduce((total, seat) => total + seat.giaVe, 0);
  };

  const handleProceed = () => {
    if (activeSeats.length === 0) {
      alert(
        "You haven't selected any seats. Please choose your seats before proceeding with the booking process."
      );
    } else {
  ...
  const handleConfirm = () => {
    console.log("Thông tin đẩy qua checkout là:", maLichChieu, activeSeats);
  }
      ...
      <div className="container mx-auto px-4 flex justify-between items-start gap-5">
        ...
        <div className="w-5/7">
          ...
          <div className="flex flex-col items-center bg-gray-200 p-6 gap-6">
            {/* SCREEN */}
            <div
              className="relative w-full py-3 text-center text-white font-extrabold text-xl tracking-wider border-5 border-b border-gray-500"
              style={{
        ...
        <div className="w-2/7">
          <div className="bg-[#555555] text-white p-7 rounded-3xl shadow-xl flex flex-col gap-6 border border-gray-100 transition-all hover:shadow-2xl">
            <div>{renderMovieInformation()}</div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 py-2">
                  ...
                  <span className="text-sm font-semibold text-orange-500 ml-auto text-right">
                    {dataSeats.thongTinPhim.tenCumRap}
                  </span>
                </div>
                ...
                <div className="flex items-start gap-3 py-2">
                  <i className="fa-solid fa-building text-xl text-red-500"></i>
                  <span className="text-sm font-medium text-gray-600 w-32">
                    Auditorium:
                  </span>
                  <span className="text-sm font-semibold text-orange-500 ml-auto text-right max-w-[80%]">
